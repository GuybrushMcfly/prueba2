...
def obtener_comisiones():
    resp = supabase.table("vista_comisiones_abiertas").select(
        "id_comision_sai, organismo, id_actividad, nombre_actividad, fecha_desde, fecha_hasta, creditos, modalidad_cursada, link_externo, apto_tramo, fecha_cierre"
    ).execute()
...
df_temp["Apto tramo"] = df_temp["apto_tramo"]
df_temp["Créditos"] = df_temp["creditos"].fillna(0).astype(int)
df_temp["Ver más"] = df_temp["link_externo"]
df_temp["Actividad (Comisión)"] = df_temp["nombre_actividad"] + " (" + df_temp["id_comision_sai"] + ") - " + df_temp["Fecha inicio"] + " al " + df_temp["Fecha fin"]
...
df_comisiones = df_filtrado[[
    "Actividad (Comisión)", "Actividad", "Comisión", "Fecha inicio", "Fecha fin", "Fecha cierre", "Créditos", "Modalidad", "Apto tramo", "Ver más"
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            html += f'<tr onclick="{onclick_code}">' \
                     ...
                     f'<td class="fecha-col">{row["Fecha inicio"]}</td>' \
                     f'<td class="fecha-col">{row["Fecha fin"]}</td>' \
                     f'<td class="creditos-col">{row["Créditos"]}</td>'
...
actividad_elegida = st.selectbox("Actividad (Comisión)", actividad_list)

if actividad_elegida in df_temp["Actividad (Comisión)"].values:
    ...
    # ========== CUIL ==========
    cuil = st.text_input("👉 Ingresá tu CUIL sin guiones ni espacios:")
    if cuil:
        if not validar_cuil(cuil):
            st.error("❌ El CUIL ingresado no es válido.")
        else:
            st.success("✅ CUIL válido. Continuá con el formulario.")
